...
class Sina2Spider(scrapy.Spider):
    ...
    def __init__(self, page=None, flag=None, *args, **kwargs):
        ...
        self.page = int(page)
        self.flag = int(flag)
        self.start_urls = ['https://news.sina.com.cn/china/', 'https://ent.sina.com.cn/film/',
    ...
    def parse(self, response):
        ...
        for i in range(self.page):
            ...
            for i in range(len(title)):
                ...
                if self.flag == 1:
                    ...
                    if yesterday <= item['times'].strftime("'%Y-%m-%d'") < today:
                        ...
                        yield Request(url=response.urljoin('href'), meta={"name": item},
                                      callback=self.parse_namedetail)
            ...
            try:
                driver.find_element_by_xpath('//*[@id="feedCardContent"]//span[@class="pagebox_next"]').click()
            except:
                break
